...
import { createTennisColumns } from "@/components/tables/columns/tennis-columns";
...
const COLUMN_STORAGE_KEY = "oddstracker_columns_tennis";

const MARKET_TYPE_OPTIONS = [
  { id: "MONEYLINE", name: "Moneyline" },
  { id: "HANDICAP", name: "Set Handicap" },
  { id: "TOTAL", name: "Total Games" },
];

export default function TennisPage() {
  const { fixtures, loading: fixturesLoading, error, isDemoData } = useFixtures("tennis");
  const { markets, loading: marketsLoading } = useMarkets("tennis");
  const { filters, updateFilter, resetFilters } = useFilters({ sportSlug: "tennis" });
  ...
  const columns = React.useMemo(() => {
    return createTennisColumns(markets);
  }, [markets]);
  ...
  const toolbarRenderer = React.useCallback(
    (table: TanstackTable<FixtureWithEnrichedOdds>) => (
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <ExportButtons table={table} filename="tennis" />
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <p className="text-sm uppercase tracking-[0.3em] text-primary/70">Tableau Tennis</p>
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-slate-900">🎾 Tennis</h1>
            <p className="text-sm text-muted-foreground">
              Moneylines, handicaps sets et totaux jeux pour les tournois majeurs.
            </p>
            {isDemoData && (
              <p className="mt-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                Données démo chargées pour les tests. Remplacez-les par les données réelles pour la prod.
              </p>
            )}
          </div>
