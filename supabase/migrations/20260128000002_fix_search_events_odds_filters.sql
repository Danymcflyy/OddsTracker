-- Migration: Fix search_events function to support specific odds filtering (JSONB)
-- Apply this manually in Supabase SQL Editor if automatic migration fails.
...
DROP FUNCTION IF EXISTS search_events(text, timestamptz, timestamptz, text, text, numeric, numeric, numeric, numeric, text, text, numeric, numeric, text, integer, integer, integer, jsonb);

-- Create the new function with JSONB support for odds filters
CREATE OR REPLACE FUNCTION search_events(
  ...
  p_odds_filters JSONB DEFAULT NULL -- The new parameter for column-specific filters
)
RETURNS TABLE (
  id UUID,
  sport_key TEXT,
...
BEGIN
  ...
  filtered_events AS (
    ...
    WHERE
      -- 1. Apply Legacy Filters (Global Odds Filters) combined with NEW Specific Odds Filters
      (
        -- Global Opening Odds Filter
        (
                  ...
                  WHERE
                    ...
                    AND (
                      ...
                      (
                        ...
                        AND (ms->'opening_odds'->>(f->>'outcome'))::numeric >= COALESCE((f->>'min')::numeric, 0)
                  ...
                  WHERE
                    ...
                    AND (
                      ...
                      (
                        ...
                        AND (mval->>(f->>'outcome'))::numeric >= COALESCE((f->>'min')::numeric, 0)
...
COMMENT ON FUNCTION search_events IS 'Advanced search with separate opening/closing odds ranges and specific column filters.';
