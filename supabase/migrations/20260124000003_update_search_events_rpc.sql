-- Update search_events function with real JSON filtering logic
...
BEGIN
  ...
  WHERE 
    -- Advanced Odds Filtering
    (p_odds_min IS NULL AND p_odds_max IS NULL AND p_point_value IS NULL) 
    OR (
        EXISTS (
            SELECT 1 
            ...
            WHERE 
                ...
                AND
                (p_outcome IS NULL OR odds.key = p_outcome)
                AND
                (jsonb_typeof(odds.value) = 'number')
                AND
                (p_odds_min IS NULL OR odds.value::numeric >= p_odds_min)
                AND
                (p_odds_max IS NULL OR odds.value::numeric <= p_odds_max)
        )
    )
